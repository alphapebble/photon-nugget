    ...
    with gr.Blocks(title="RAG Evaluation Dashboard") as dashboard:
        ...
        with gr.Row():
            ...
            with gr.Accordion("Detailed Results", open=False):
                detailed_results = gr.Dataframe(
                    headers=["Question", "Response", "Metrics"],
                    interactive=False
                )
        ...
        def update_dashboard(results_dir):
            ...
            if not detailed_df.empty:
                ...
                display_cols = ["Question", "Response", "Keyword Match %"]
                
                # Add RAGAS metrics if available
                ragas_cols = [col for col in detailed_df.columns if col.startswith("RAGAS ")]
                display_cols.extend(ragas_cols)
                
                # Add other metrics if available
                other_metrics = ["BLEU Score", "ROUGE-1 F1", "Response Time (s)"]
                ...
                detailed_display = detailed_df[display_cols].to_dict('records')
            else:
                detailed_display = []
